...
function _crearPedidoWhatsapp(data) {
  ...
  if (idx["ULTIMA_EDICION"] == null) {
    idx["ULTIMA_EDICION"] = _ensureHeader_(sh, "ULTIMA_EDICION");
    headers.push("ULTIMA_EDICION");
  }
  ...
  row[idx["ULTIMA_EDICION"]] = new Date();
...
function _claveEdicionEnvioRetiro_() {
  try {
    return (
      PropertiesService.getScriptProperties().getProperty("CLAVE_EDICION_ENVIO_RETIRO") ||
      CLAVE_EDICION_ENVIO_RETIRO_FALLBACK
    );
  } catch (err) {
    return CLAVE_EDICION_ENVIO_RETIRO_FALLBACK;
  }
}
...
function _cambiarEnvioRetiro(data) {
  ...
  const sh = _getSheet_();
  const idPedido = String(data.id_pedido || "").trim();
  const sucursalActual = String(data.sucursal || "").toUpperCase().trim();
  const usuario = String(data.usuario || "").trim();
  const tipoEnvioNuevo = String(data.tipo_envio || "").toUpperCase().trim();
  const sucursalRetiroNueva = String(data.sucursal_retiro || "").toUpperCase().trim();

  if (!idPedido || !sucursalActual || !tipoEnvioNuevo || !sucursalRetiroNueva || !usuario) {
    return _jsonError("Faltan datos (id_pedido / sucursal / tipo_envio / sucursal_retiro / usuario)");
  }
  ...
  for (let i = 1; i < values.length; i++) {
    const row = values[i];
    const idHoja = String(row[idx["ID_PEDIDO"]] || "").trim();
    const sucHoja = String(row[idx["SUCURSAL_RETIRO"]] || "").toUpperCase().trim();

    if (idHoja === idPedido && _sucursalCoincide_(sucHoja, sucursalActual)) {
      const fila = i + 1;
      const tipoEnvioAntes = row[idx["TIPO_ENVIO"]] || "";
      const sucursalAntes = row[idx["SUCURSAL_RETIRO"]] || "";

      sh.getRange(fila, idx["TIPO_ENVIO"] + 1).setValue(tipoEnvioNuevo);
      sh.getRange(fila, idx["SUCURSAL_RETIRO"] + 1).setValue(sucursalRetiroNueva);

      if (idx["QUIEN_REGISTRA"] != null) {
        sh.getRange(fila, idx["QUIEN_REGISTRA"] + 1).setValue(usuario);
      }
      ...
      _logCambioEstado_({
        ...
        estado_antes: row[idx["ESTADO"]] || "",
        estado_despues: row[idx["ESTADO"]] || "",
        tipo_envio: tipoEnvioNuevo,
        canal: row[idx["CANAL"]] || "",
        obs: "Envio/retiro: " + tipoEnvioAntes + " / " + sucursalAntes + " => " + tipoEnvioNuevo + " / " + sucursalRetiroNueva
      });
      ...
      return _jsonOk({
        ...
        fila,
        tipo_envio: tipoEnvioNuevo,
        sucursal_retiro: sucursalRetiroNueva
      });
    }
  }

  return _jsonError("Pedido no encontrado para esa sucursal actual");
}
